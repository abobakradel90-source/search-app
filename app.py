...
# ==========================================
# ✅ محتوى الموقع (معزول داخل else لمنع أي تداخل)
# ==========================================
else:
    ...
    if st.session_state.current_user == "abobakr":
        with st.sidebar:
            ...
            # 🌟 إزالة الـ Form من أداة الرفع لضمان الاستجابة الفورية 🌟
    ...
    with main_tab2:
        ...
        if not shared_inv_state.get("is_active", False):
            ...
            # 🌟 إزالة الـ Form لفتح الجلسة لضمان الاستجابة 🌟
    ...
    with main_tab3:
        ...
        if not shared_sales.get("is_active", False):
            ...
            # 🌟 إزالة الـ Form لفتح الوردية لضمان الاستجابة 🌟
    ...
    with main_tab_cat:
        ...
        st.markdown("يعرض هذا الكاتالوج الأصناف المتوفرة فقط بالمخزن مع تفاصيل الرصيد والأسعار.")
        ...
        # 💡 السر لتجنب الانهيار: وضع صورة شفافة 1x1 بيكسل للأصناف التي لا تمتلك صورة
        ...
        if catalog_data:
            ...
            st.dataframe(
                df_catalog,
                column_config={
                    "صورة": st.column_config.ImageColumn("صورة"),
                    "سعر القطعة": st.column_config.NumberColumn("السعر (ج.م)", format="%.2f"),
                    "الرصيد الدفتري": st.column_config.NumberColumn("الرصيد الدفتري"),
                    "المبيعات": st.column_config.NumberColumn("المبيعات بالوردية"),
                    "الرصيد المتاح": st.column_config.NumberColumn("الرصيد المتاح")
                },
                use_container_width=True,
                hide_index=True
            )

            df_excel_cat = df_catalog.drop(columns=["صورة"])
            buf_cat = io.BytesIO()
