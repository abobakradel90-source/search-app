...
import re
import time
import traceback  # مكتبة الصندوق الأسود لاصطياد الأخطاء
...
try:
    ...
    with main_tab2:
        ...
        if not shared_inv_state.get("is_active", False):
            ...
            out_of_stock = sum(1 for info in system_inventory.values() if info.get('sys_stock', 0) <= 0)
            ...
            with inv_tab2:
                ...
                if scan_code:
                    ...
                    if found_code:
                        ...
                        render_product_card(found_code, system_inventory[found_code].get('name',''), system_inventory[found_code].get('sys_stock',0))
    ...
    with main_tab3:
        ...
        if not shared_sales.get("is_active", False):
            ...
            total_revenue = sum(float(inv.get("total", 0.0)) for inv in all_invoices)
            total_invoices_count = len(all_invoices)
            my_invoices = sum(float(inv.get("total", 0.0)) for inv in all_invoices if str(inv.get("salesperson", "")) == st.session_state.current_user)
            ...
            if not st.session_state.active_customer:
                ...
                if all_invoices:
                    ...
                    for idx, inv in enumerate(all_invoices):
                        ...
                        with st.expander(f"🧾 فاتورة #{inv_id} | العميل: {inv.get('customer','')} | الإجمالي: {inv.get('total',0)} ج.م"):
                            df_v = pd.DataFrame(inv.get('items', []))
                            if not df_v.empty: st.dataframe(df_v[['code', 'name', 'qty', 'price', 'total']], use_container_width=True)
                            
                            col_e, col_d = st.columns(2)
                            with col_e:
                                if st.button(f"✏️ تعديل ومتابعة إضافة الأصناف", key=f"edit_{inv_id}"):
                                    l_sales = load_shared_sales()
                                    if "deductions" not in l_sales: l_sales["deductions"] = {}
                                    for item in inv.get('items', []): 
                                        c_code = item.get("code", "")
                                        l_sales["deductions"][c_code] = l_sales["deductions"].get(c_code, 0) - item.get("qty", 0)
                                    l_sales["invoices"] = [i for i in l_sales.get("invoices", []) if i.get("invoice_id") != inv_id]
                                    save_shared_sales(l_sales)
                                    
                                    st.session_state.active_customer = inv.get('customer', '')
                                    st.session_state.invoice_cart = inv.get('items', [])
                                    st.session_state.active_invoice_id = inv_id
                            ...
                            with col_d:
                                if not df_v.empty:
                                    ...
                                    st.download_button("📥 تحميل الإكسيل مجدداً", data=buf.getvalue(), file_name=f"Invoice_{inv_id}_{inv.get('customer','')}.xlsx", key=f"dl_{inv_id}")
                    ...
                    with col_cancel:
                        ...
                        if st.button(btn_text):
                            if st.session_state.active_invoice_id and st.session_state.original_invoice:
                                ...
                                l_sales["invoices"].sort(key=lambda x: x.get("invoice_id", 0))
                                for item in st.session_state.original_invoice.get("items", []):
                                    c_code = item.get("code", "")
                                    l_sales["deductions"][c_code] = l_sales["deductions"].get(c_code, 0) + item.get("qty", 0)
                                save_shared_sales(l_sales)
    ...
    with main_tab_cat:
        ...
        st.markdown("يعرض هذا الكاتالوج الأصناف المتوفرة فقط بالمخزن مع أرصدتها اللحظية وأسعارها الحالية.")
        ...
        if catalog_data:
            ...
            with pd.ExcelWriter(buf_cat, engine='openpyxl') as w: 
                df_excel_cat.to_excel(w, index=False, sheet_name='كاتالوج المنتجات المتاحة')
                
            st.download_button(
                label="📥 تحميل الكاتالوج كملف إكسيل (Excel)",
                data=buf_cat.getvalue(),
                file_name=f"Live_Catalog_With_Prices_{datetime.date.today()}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                type="primary"
            )
        else:
        ...
        with main_tab4:
                ...
                for inv in rec.get('invoices', []):
                    for item in inv.get('items', []):
                        flat_records.append({
                            ...
                            "الكمية": float(item.get('qty', 0)),
                            "سعر الجملة": float(item.get('price', 0.0)),
                            "الإجمالي": float(item.get('total', 0.0))
                        })
            ...
            if not flat_records:
                ...
                with col_chart:
                    ...
                    if 'البائع' in df_all_sales.columns:
                ...
                with col_table:
                    ...
                    if 'كود الصنف' in df_all_sales.columns and 'اسم الصنف' in df_all_sales.columns:
                ...
                if 'اسم العميل' in df_all_sales.columns:
                    customer_report = df_all_sales.groupby('اسم العميل').agg({'الكمية': 'sum', 'الإجمالي': 'sum'}).reset_index().sort_values(by='الإجمالي', ascending=False)
    ...
    # 🔴 الصندوق الأسود (يمنع الشاشة الحمرا المزعجة ويقولك المشكلة فين بالظبط)
    st.error("🚨 عذراً يا هندسة، حدث تعارض في البيانات بسبب ملفات قديمة!")
    st.warning("برجاء تصوير هذه الشاشة وإرسالها للمبرمج فوراً لحلها:")
